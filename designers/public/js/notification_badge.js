(function () {
	...
	const SELECTOR_ITEM = ".standard-items-sections .sidebar-notification .item-anchor";
	const SELECTOR_PANEL = ".dropdown-notifications .panel-notifications";
	...
	function getBadgeContainer() {
		const item = document.querySelector(SELECTOR_ITEM);
		if (!item) return null;
	...
	function getUnreadFromVisiblePanel() {
		const panel = document.querySelector(SELECTOR_PANEL);
		if (!panel) return null;
		const unread = panel.querySelectorAll(".notification-item.unread").length;
		return Number.isFinite(unread) ? unread : null;
	}
	...
	function refreshFromServer() {
		...
		fetchLiveNotifications(100)
			.then((r) => {
				...
				const unread = logs.reduce((acc, row) => acc + (row?.read ? 0 : 1), 0);
				renderCount(unread);
	...
	function refreshFastThenServer() {
		const fromPanel = getUnreadFromVisiblePanel();
		if (fromPanel !== null) renderCount(fromPanel);
		refreshFromServer();
		syncDropdownList();
	}
	...
	function observePanelChanges() {
		const panel = document.querySelector(SELECTOR_PANEL);
		if (!panel || panel.__designersObserved) return;
		panel.__designersObserved = true;
